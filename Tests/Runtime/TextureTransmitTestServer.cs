...
namespace DreemurrStudio.Network.DEMO
{
    ...
    /// 贴图传送测试的服务器端脚本
    /// </summary>
    public class TextureTransmitTestServer : MonoBehaviour
    {
        [SerializeField]
        [Tooltip("要发送的纹理")]
        ...
        /// 受到图像数据时的回调
        /// </summary>
        /// <param name="clientIPEP">发送图像数据的客户端</param>
        /// <param name="imageData">图形的二进制数据</param>
        private void OnDataReceived(IPEndPoint clientIPEP, byte[] imageData)
        {
            Debug.Log($"收到来自{clientIPEP}的图像数据，大小:{imageData.Length}bytes");
